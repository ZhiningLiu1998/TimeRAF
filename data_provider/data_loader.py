...
def _load_hf_dataframe(cfg_name):
    if load_dataset is None:
        raise ImportError(
            "datasets is required to auto-download benchmark data. "
            "Install it or place the dataset file locally under root_path."
        )
...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        if os.path.exists(local_fp):
            ...
            df_raw = _load_hf_dataframe(cfg_name)
...
class Dataset_PEMS(Dataset):
    """TimeFuse's PEMS forecasting split and evaluation window protocol."""
    ...
    def __init__(self, args, root_path, flag='train', size=None,
        ...
        if flag not in type_map:
            raise ValueError(f"Unsupported PEMS split: {flag}")
        self.set_type = type_map[flag]
        self.scale = scale
        self.root_path = root_path
        self.data_path = data_path
    ...
    def __read_data__(self):
        raw = np.load(os.path.join(self.root_path, self.data_path), allow_pickle=True)
        data = raw['data'][:, :, 0]
        train_end = int(0.6 * len(data))
        val_end = int(0.8 * len(data))
        splits = (data[:train_end], data[train_end:val_end], data[val_end:])
        data = splits[self.set_type]

        self.scaler = StandardScaler()
        if self.scale:
            self.scaler.fit(data)
            data = self.scaler.transform(data)
        self.data_x = pd.DataFrame(data).ffill().bfill().values
        self.data_y = self.data_x

    def __getitem__(self, index):
        s_begin = index * self.test_stride if self.set_type == 2 else index
        s_end = s_begin + self.seq_len
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        seq_x = self.data_x[s_begin:s_end]
        seq_y = self.data_y[r_begin:r_end]
        seq_x_mark = torch.zeros((len(seq_x), 1))
        seq_y_mark = torch.zeros((len(seq_y), 1))
        return seq_x, seq_y, seq_x_mark, seq_y_mark

    def __len__(self):
        windows = len(self.data_x) - self.seq_len - self.pred_len + 1
        return windows // self.test_stride if self.set_type == 2 else windows
